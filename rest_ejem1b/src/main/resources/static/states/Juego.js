...
Doors.Juego = function(game){
	
}
Doors.Juego.prototype = {
		preload:function(){
		...
		create:function(){
			...
			p0 = this.Puerta(50,215,0, 0);	
			p1 = this.Puerta(300,200,0, 1);
			p2 = this.Puerta(550,215,0, 2);
			p3 = this.Puerta(800,200,0, 3);
			puertas = [p0, p1, p2, p3];
			...
			//ICONOS
			i0 = this.Icono(10,10, Math.floor((Math.random() * 23)));
			i1 = this.Icono(110,10, Math.floor((Math.random() * 23)));
			i2 = this.Icono(210,10, Math.floor((Math.random() * 23)));
			iconos = [i0, i1, i2];
	...
	Puerta: function(x, y, tipo, ind){
		...
		this.spritePuerta = this.add.sprite(this.x, this.y, 'puerta0');

		this.spritePuerta.events.onInputDown.add(Doors.Juego.prototype.atravesarPuerta,Doors.Juego.prototype,0,this);
	 ...
	 remarcarIcono: function(sprite, cursor, indice){
		if(jugador == 1){
			for(var i = 0; i < iconos.length; i++){
				iconos[i].resetIcono(iconos[i].getIndice());
			}
